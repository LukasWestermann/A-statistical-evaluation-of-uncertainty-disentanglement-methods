"""
Recompute AU/EU via Monte Carlo mixture entropy (numerical) from saved regression raw_outputs .npz.

Single file:
    python scripts/recompute_entropy_numerical_batch_from_npz.py [path/to/file.npz] [--n-samples 5000] [--seed 0]
...
# Excel knn_recompute_note column (schema); values identify MC mixture recompute
NOTE_OOD = "NLL/CRPS/Spearman not in raw_outputs npz; MC mixture entropy (numerical)"
NOTE_SAMPLE_SIZE = "from raw_outputs MC mixture (numerical)"
NOTE_NOISE_LEVEL = "from raw_outputs MC mixture (numerical)"
NOTE_UNDERSAMPLING = (
    "undersampling npz has single grid; not split by sampling regions; MC mixture entropy (numerical)"
)


def _default_out_root() -> Path:
    return project_root / "results" / "entropy_recomputed_numerical"
...
def run_batch(
    ...
    print(
        "Batch numerical entropy: total work scales with n_samples × M × N (grid chunking limits RAM). "
        "Use --grid-stride, smaller --n-samples, or --grid-chunk-size if needed.",
        flush=True,
    )
        ...
        for func_type, noise_type in CONDITIONS_4:
            ...
            if key == "ood":
                for npz_path in all_npz:
                    ...
                    save_stats_excel(df, out_sub, f"{date}_{mp}_numerical_entropy_{sanitize_stem(npz_path.stem)}")
                    if per_npz_plots:
                ...
                for model_tag, paths in by_model.items():
                    ...
                    for npz_path in paths:
                        try:
                            res = compute_numerical_grid_result(
                                npz_path, n_samples, seed, [], grid_stride, grid_chunk_size
                            )
                        except Exception as e:
                            print(f"[{key}] skip {npz_path.name}: {e}")
                            continue
                    ...
                    out_sub = stats_root / key / noise_type / func_type
                    save_stats_excel(df, out_sub, f"{date}_{mp}_numerical_entropy_sample_size")
                ...
                for (model_tag, dist), paths in by_grp.items():
                    ...
                    out_sub = stats_root / key / noise_type / func_type
                    save_stats_excel(df, out_sub, f"{date}_{mp}_numerical_entropy_noise_{dist}")
            ...
            for _disp, _tag, globs in MODEL_RESOLVERS:
                ...
                try:
                    res = compute_numerical_grid_result(
                        p, n_samples, seed, ranges_for_entropy, grid_stride, grid_chunk_size
                    )
            ...
            create_2x4_variance_panel(
                ...
                pdir / f"panel_{key}_numerical_2x4_{slug}_variance.png",
                title_short,
                ranges_for_panels,
            )
            create_2x4_entropy_panel(
...
def main():
    parser = argparse.ArgumentParser(
        description="MC mixture (numerical) entropy from raw_outputs .npz (batch or single file)"
    )
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="N",
        help="Mixture log-pdf grid columns per block (default: auto ~256 MiB)",
    )
