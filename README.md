...
**Why a separate “recomputation” path exists:** Some historical runs were produced with a different initially wrong sllighty wrong impleantion of the `entropy_method` in the regression case, and this mismatch was only noticed after many models had already been trained. Re-training all models just to refresh entropy columns was computationally very expensive. The batch script therefore **recomputes moment-matched entropy from existing `*raw_outputs*.npz`**, whcih were not affected by the wrong entropy formula, and writes **parallel** statistics and plots under a dedicated tree (below). New notebook runs, with the current corrected defaults, should already write consistent moment-matched entropy into the normal experiment statistics; the batch remains useful for **refreshing plots/tables from old npz** without rerunning training.
